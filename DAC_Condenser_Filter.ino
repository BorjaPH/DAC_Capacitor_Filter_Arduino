...
void printSetOutput() {
  Serial.println("Please, enter the output to generate --> Output [value]");
  Serial.println("Please, remeber that the output value must be between 0-5000 mV");  
  while (!Serial.available()) {}
  String command = Serial.readStringUntil('\n');
  if (command.startsWith("Output ")) {
    output = command.substring(7).toInt();
    Serial.print("Value 1 = "); Serial.println(output);
    if (output >= 0 && output <= 5000 ) {
      duty = map(output,0,5000,0,255);
      return; 
    }else {
...
void SetTimer2() {
  ...
  OCR2A=duty; // this is Arduino UNO pin 11 
}
